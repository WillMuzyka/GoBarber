...
let ListProvidersService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    let providers = await this.cacheProvider.recover(`providers_list:${except_user_id}`);

    if (!providers) {
      providers = await this.usersRepository.findAllProviders({
        except_user_id
      });
      await this.cacheProvider.save(`providers_list:${except_user_id}`, (0, _classTransformer.classToClass)(providers));
    }
