...
let CreateAppointmentService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (user_id === provider_id) throw new _AppError.default("You can't make an appointment with yourself");

    if ((0, _dateFns.isBefore)(appointmentDate, Date.now())) {
      throw new _AppError.default("You can't make an appointment at past dates");
    }

    const appointmentHour = (0, _dateFns.getHours)(appointmentDate);
    if (appointmentHour < 8 || appointmentHour > 17) throw new _AppError.default('You can only make an appointment from 8am to 6pm');
    ...
    const formattedDate = (0, _dateFns.format)(appointmentDate, "'dia' dd/MM/yyyy 'às' HH:mm'h'");
    await this.notificationsRepository.create({
      recipient_id: provider_id,
      content: `Novo agendamento ${formattedDate}`
    });
    await this.cacheProvider.invalidate(`provider_appointments:${provider_id}:${(0, _dateFns.format)(appointmentDate, 'yyyy-M-d')}`);
    return appointment;
  }
