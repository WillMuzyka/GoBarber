...
let UpdateProfileService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (userWithSameEmail && userWithSameEmail.id !== user_id) {
      throw new _AppError.default('Email already being used');
    }
    ...
    if (password) {
      if (!old_password) throw new _AppError.default('Need to inform your old password');
      const matchOldPassword = await this.hashProvider.compareHash(old_password, user.password);
      if (!matchOldPassword) throw new _AppError.default('Old password does not match');
